...
func TestServerRegister(t *testing.T) {
	...
	t.Run("normal register", func(t *testing.T) {
		...
		s.Register(&TestReq{}, handler)
	})

	t.Run("duplicate register should panic", func(t *testing.T) {
		defer func() {
			if r := recover(); r == nil {
	...
	t.Run("nil message should panic", func(t *testing.T) {
		defer func() {
			if r := recover(); r == nil {
				t.Error("Expected panic for nil message")
			}
		}()
		handler := func(ci *CallInfo) {}
		s.Register(nil, handler)
	})
...
func TestServerCast(t *testing.T) {
	...
	s.Register(&TestReq{}, func(ci *CallInfo) {
		received <- true
	})
...
func TestClientNew(t *testing.T) {
	...
	t.Run("client", func(t *testing.T) {
		...
		if c.chanSyncRet == nil {
			t.Error("Client.chanSyncRet is nil")
		}
		if c.ChanAsynRet == nil {
...
func TestClientSyncCall(t *testing.T) {
	...
	t.Run("call with error", func(t *testing.T) {
		s.Register(&TestReq2{}, func(ci *CallInfo) {
...
// TestClusterCast 测试集群Cast
func TestClusterCast(t *testing.T) {
	s := NewServer()
	received := make(chan bool, 1)

	s.Register(&TestReq{}, func(ci *CallInfo) {
		if ci.SrcNodeType != "test-node" {
			t.Errorf("SrcNodeType = %v, want %v", ci.SrcNodeType, "test-node")
		}
		if ci.SrcServerID != 123 {
			t.Errorf("SrcServerID = %v, want %v", ci.SrcServerID, 123)
		}
		received <- true
	})

	err := s.ClusterCast("test-node", 123, &TestReq{Value: 10})
	if err != nil {
		t.Errorf("ClusterCast() error = %v", err)
	}
	...
	case <-received:
		// 成功
	case <-time.After(1 * time.Second):
		t.Error("ClusterCast message not received")
	}
}
