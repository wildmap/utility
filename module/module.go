...
func (a *App) runModule(wrapper *moduleWrapper) {
	defer func() {
		wrapper.wg.Done()
		if r := recover(); r != nil {
			slog.Error(fmt.Sprintf("module panic recovered, moudle %s, panic %v", wrapper.module.Name(), r))
		}
	}()
...
func (a *App) destroyModule(wrapper *moduleWrapper) {
	defer func() {
		if r := recover(); r != nil {
			slog.Error(fmt.Sprintf("module destroy panic recovered %s, panic %v", wrapper.module.Name(), r))
		}
	}()
