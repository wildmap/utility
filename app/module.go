...
type IModule interface {
	Name() string             // 名称
	OnInit() error            // 初始化
	Run(closeSig chan bool)   // 运行, 阻塞
	PostRun()                 // 运行后, 运行后执行一些其他操作, 不能阻塞
	OnDestroy()               // 销毁
...
func (a *App) Register(mod IModule) error {
	...
	a.modules = append(a.modules, &moduleWrapper{
		...
		runStarted: make(chan bool, runStartedBufferSize),
	})
	a.Unlock()
	return nil
}
...
// runModule 启动模块
func (a *App) runModule(wrapper *moduleWrapper, dynamic bool) {
	...
	// Notify that Run() has started
	select {
	case wrapper.runStarted <- true:
	default:
	}
...
func (a *App) postRunModule(wrapper *moduleWrapper) {
	defer func() {
		if r := recover(); r != nil {
			xlog.Errorf("module %s panic recovered, moudle, panic %v\n%s", wrapper.module.Name(), r, string(debug.Stack()))
		}
	...
	moduleName := wrapper.module.Name()
	xlog.Infof("running module %s PostRun", moduleName)
	wrapper.module.PostRun()
	xlog.Infof("module %s PostRun complete", moduleName)
}
