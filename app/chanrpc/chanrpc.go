...
func (ci *CallInfo) Ret(ret any) {
	...
	if !ci.hasRet.CompareAndSwap(false, true) {
		slog.Error(fmt.Sprintf("chanrpc msgid %d can not ret twice, %s", ci.id, string(debug.Stack())))
		return
	}

	// 封装参数 执行回调
	if err := ci.ret(&RetInfo{Ack: ret}); err != nil {
		slog.Error(fmt.Sprintf("chanrpc msgid %d ret error %v", ci.id, err))
	}
}
...
func (ci *CallInfo) ret(ri *RetInfo) (err error) {
	...
	defer func() {
		if r := recover(); r != nil {
			...
			slog.Error(fmt.Sprintf("chanrpc msgid %d ret err %v\n%s", ci.id, err, string(debug.Stack())))
		}
	}()
	// 封装参数 将结果信息放入返回通道
	ri.cb = ci.cb
...
type Client struct {
	...
	callList        map[int64]any  // 待处理异步调用
...
func (s *Server) exec(ci *CallInfo) (err error) {
	defer func() {
		if r := recover(); r != nil {
			...
			slog.Error(fmt.Sprintf("chanrpc msgid %d exec panic %v\n%s", err, ci.id, string(debug.Stack())))
...
func (s *Server) Exec(ci *CallInfo) {
	if ci == nil {
		slog.Error(fmt.Sprintf("chanrpc exec: CallInfo is nil"))
		return
	}
	ci.hasRet.Store(false)
	if err := s.exec(ci); err != nil {
		slog.Error(fmt.Sprintf("chanrpc msgid %d exec error %v", ci.id, err))
	}
}
...
func NewClient(callLen int) *Client {
	c := &Client{
		...
		callList:    make(map[int64]any, callLen),
	}
	return c
}
...
func execCb(ri *RetInfo) {
	defer func() {
		if r := recover(); r != nil {
			slog.Error(fmt.Sprintf("chanrpc  callback panic: %v\n%s", r, string(debug.Stack())))
		}
	}()
...
func (c *Client) Close() {
	...
	if !atomic.CompareAndSwapInt32(&c.closed, 0, 1) {
		slog.Warn(fmt.Sprintf("chanrpc client already closed"))
		return
	}

	pending := atomic.LoadInt32(&c.pendingAsynCall)
	slog.Info(fmt.Sprintf("closing chanrpc client pending_calls %d", pending))
	...
	go func() {
		...
		for {
			...
			case <-timeout:
				...
				slog.Warn(fmt.Sprintf("chanrpc client close timeout remaining_calls %d", remaining))

				// 打印剩余的调用信息
				for sid, _call := range c.callList {
					slog.Warn(fmt.Sprintf("pending call not finished session_id %d call %d", sid, _call))
				}
	...
	// 等待goroutine完成
	wg.Wait()
	slog.Info(fmt.Sprintf("chanrpc client closed successfully"))
}
