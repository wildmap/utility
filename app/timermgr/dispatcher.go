...
func (disp *Dispatcher) run() {
	...
	lastTick := xtime.ToUTC(xtime.Now()).UnixNano() / 1e6 / timerTick
	...
	for {
		...
		case <-tickTimer.C:
			...
			lastTick = disp.doTick(xtime.ToUTC(xtime.Now()), lastTick)
		}
	}
}
...
func (disp *Dispatcher) place(t *dispatcherTimer) {
	diff := t.endTs - xtime.ToUTC(xtime.Now()).UnixNano()/1e6
	if diff <= 0 {
...
func (disp *Dispatcher) doTick(now time.Time, lastTick int64) int64 {
	...
	nowTick := xtime.ToUTC(xtime.Now()).UnixNano() / 1e6 / timerTick
