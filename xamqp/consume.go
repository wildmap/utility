...
func (consumer *Consumer) handlerWrapper(consumeOptions ConsumerOptions, handler Handler, msg amqp.Delivery) {
	defer func() {
		if r := recover(); r != nil {
			...
			// Try to nack if possible, or just log.
			// If AutoAck is false, we should probably Nack to avoid message loss,
			// but if Nack fails (channel closed), it's fine.
			if !consumeOptions.RabbitConsumerOptions.AutoAck {
				_ = msg.Nack(false, false) // discard or requeue? discard for safety against poison pills
			}
		}
	}()
